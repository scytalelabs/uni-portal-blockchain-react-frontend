...
class DeanVerifyAssessmentByCourse extends Component{
      ...
      Verify(state){
        ...
        //   console.log('dean/'+state.semester+'/'+state.course+'/'+state.section+'/approved_assessments');
          axios.get(baseUrl+'dean/'+state.semester+'/'+state.course+'/has_approved_assessments')
          .then( res => {
              console.log("courses:",res);
              if(res.data.status==true)
              {
                    axios.defaults.headers.common['Authorization']=token;
                    // console.log('dean/'+state.semester+'/'+state.course+'/verify_all_assessments/specific_course');
                    
                    axios.get(baseUrl+'dean/'+state.semester+'/'+state.course+'/verify_all_assessments/specific_course')
                    .then( res => {
                        console.log("courses:",res);
                        this.setState({
                            verify:res.data,
                                 ...
                                 <Row className='form-group'>           
                                     <Col md={{offset:1}}>
                                         <div className='EditBox'style={{marginBottom:'11px'}}>
                                             <h3>Verify Assessment By Course</h3>
                                             <br></br>
                                                 <Row>
                                                 <Col>
                                                     Semester:
    ...
    export default connect(mapStatetoProps,mapDispatchtoProps)(DeanVerifyAssessmentByCourse);