...
class DeanVerifyAssessmentBySection extends Component{
      ...
      Verify(state){
        const token=localStorage.getItem('bearer_token');
          ...
          .then( res => {
              ...
              {
                    axios.defaults.headers.common['Authorization']=token;
                    console.log('dean/'+state.semester+'/'+state.course+'/'+state.section+'/verify_all_assessments/specific_section');
                    
                    axios.get(baseUrl+'dean/'+state.semester+'/'+state.course+'/'+state.section+'/verify_all_assessments/specific_section')
                    .then( res => {
                        console.log("courses:",res);
                        this.setState({
                            verify:res.data,
                                 ...
                                 <Row className='form-group'>           
                                     <Col md={{offset:1}}>
                                         <div className='EditBox'style={{marginBottom:'11px'}}>
                                             <h3>Verify Assessment By Section</h3>
                                             <br></br>
                                                 <Row>
                                                 <Col>
                                                     Semester:
    ...
    export default connect(mapStatetoProps,mapDispatchtoProps)(DeanVerifyAssessmentBySection);