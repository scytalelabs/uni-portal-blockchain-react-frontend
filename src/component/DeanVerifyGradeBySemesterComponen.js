...
class DeanVerifyGradeBySemester extends Component{
      ...
      Verify(state){
        const token=localStorage.getItem('bearer_token');
          ...
          console.log('dean/'+state.semester+'/has_approved_grades');
          axios.get(baseUrl+'dean/'+state.semester+'/has_approved_grades')
          .then( res => {
              console.log("courses:",res);
              if(res.data.status==true)
              {
                    axios.defaults.headers.common['Authorization']=token;
                    console.log('dean/'+state.semester+'/verify_grades/specific_semester');
                    
                    axios.get(baseUrl+'dean/'+state.semester+'/verify_grades/specific_semester')
                    .then( res => {
     ...
     render(){
        ...
        {
            const {semesters,semester}=this.state;
                                 ...
                                 <Row className='form-group'>           
                                     <Col md={{offset:1}}>
                                         <div className='EditBox'style={{marginBottom:'11px'}}>
                                             <h3>Verify Grade By Semester</h3>
                                             <br></br>
                                                 <Row>
                                                    <Col>
                                                        Semester:
    ...
    export default connect(mapStatetoProps,mapDispatchtoProps)(DeanVerifyGradeBySemester);